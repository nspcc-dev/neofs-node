...
// verifySHA256 checks whether b is a valid non-zero SHA-256 hash.
func verifySHA256(b []byte) error {
...
// checks whether b is a valid Neo3 address.
func verifyN3Address(b []byte) error {
...
// verifyUserIDValue checks whether b is a valid Neo3 address.
...
// verifyUUIDV4 checks whether b is a valid UUID V4.
func verifyUUIDV4(b []byte) error {
	if len(b) != uuidLen {
...
func verifyBinaryField(kind binaryFieldKind, b []byte) error {
	switch kind {
	case binaryFieldSHA256:
		return verifySHA256(b)
	case binaryFieldN3Address:
		return verifyN3Address(b)
	case binaryFieldKindUUIDV4:
		return verifyUUIDV4(b)
	default:
