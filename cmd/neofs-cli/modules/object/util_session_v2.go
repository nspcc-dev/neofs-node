...
func verbObjectToVerbV2(v1Verb session.ObjectVerb) session.VerbV2 {
	switch v1Verb {
	case session.VerbObjectGet:
		return session.VerbV2ObjectGet
	case session.VerbObjectHead:
		return session.VerbV2ObjectHead
	case session.VerbObjectPut:
		return session.VerbV2ObjectPut
	case session.VerbObjectDelete:
		return session.VerbV2ObjectDelete
	case session.VerbObjectSearch:
		return session.VerbV2ObjectSearch
	case session.VerbObjectRange:
		return session.VerbV2ObjectRange
	case session.VerbObjectRangeHash:
		return session.VerbV2ObjectRangeHash
	default:
		return session.VerbV2ObjectGet
	}
}

func getVerifiedSessionV2(cmd *cobra.Command, cmdVerb session.ObjectVerb, key *ecdsa.PrivateKey, cnr cid.ID) (*session.TokenV2, error) {
	...
	if !tok.VerifySignature() {
		return nil, errors.New("v2 session token signature verification failed")
	}

	verbV2 := verbObjectToVerbV2(cmdVerb)

	if !tok.AssertVerb(verbV2, cnr) {
		return nil, fmt.Errorf("v2 session token does not authorize %v for container %s", cmdVerb, cnr.String())
	}
	...
	if !tok.Issuer().Equals(target) {
		return nil, fmt.Errorf("V2 session token issuer %v does not match user %s", tok.Issuer(), signer.UserID().String())
	}
...
func _readVerifiedSessionV2(cmd *cobra.Command, dst SessionPrm, key *ecdsa.PrivateKey, cnr cid.ID, obj *oid.ID) error {
	...
	if obj != nil {
		verbV2 := verbObjectToVerbV2(cmdVerb)
		if !tok.AssertObject(verbV2, cnr, *obj) {
...
// tryReadSessionV2 attempts V2 token first, returns true if V2 was found.
