...
// If no file provided, creates V2 token locally.
func ReadOrOpenSessionViaClient(ctx context.Context, cmd *cobra.Command, dst SessionPrm, cli *client.Client, key *ecdsa.PrivateKey, cnr cid.ID, objs ...oid.ID) error {
	...
	err := CreateSessionV2(ctx, cmd, dst, cli, key, cnr, objs...)
	if err != nil {
		return err
	}
	return nil
...
//	*internal.PutObjectPrm
//	*internal.DeleteObjectPrm
func CreateSessionV2(ctx context.Context, cmd *cobra.Command, dst SessionPrm, cli *client.Client, key *ecdsa.PrivateKey, cnr cid.ID, objs ...oid.ID) error {
	const sessionLifetime = 36000 // 10 hours
	...
	common.PrintVerbose(cmd, "Added %d node public keys as subjects", nodeCount)
	...
	tok.SetExp(currentTime + sessionLifetime)
	tok.SetFinal(true)
...
func finalizeSessionV2(cmd *cobra.Command, dst SessionPrm, tok *sessionv2.Token, key *ecdsa.PrivateKey, cnr cid.ID, objs ...oid.ID) error {
	...
	if tok.Issuer() != signer.UserID() {
		return fmt.Errorf("user %s is not issuer of V2 session token", signer.UserID().String())
	}
