...
func initObjectDeleteCmd() {
	...
	flags.StringSlice("session-subjects", nil, "Session subject user IDs (optional, defaults to current node)")
	flags.StringSlice("session-subjects-nns", nil, "Session subject NNS names (optional, defaults to current node)")
...
func deleteObject(cmd *cobra.Command, _ []string) error {
	...
	for _, addr := range objAddrs {
		...
		err = ReadOrOpenSessionViaClient(cmd, &prm, pk, subjects, cnr, id)
		if err != nil {
			return err
		}
