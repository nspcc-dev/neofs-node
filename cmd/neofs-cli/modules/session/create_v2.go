...
func init() {
	createV2Cmd.Flags().Uint64P(commonflags.Lifetime, "l", defaultLifetimeV2, "Duration in Unix timestamps for token to stay valid")
	...
	createV2Cmd.Flags().Uint64P(commonflags.ExpireAt, "e", 0, "Expiration time in Unix timestamps for token to stay valid")
...
func createSessionV2(cmd *cobra.Command, _ []string) error {
	...
	for i, subj := range subjects {
		if subj.IsUserID() {
			common.PrintVerbose(cmd, "  Subject %d (UserID): %s", i+1, subj.UserID().String())
		} else if subj.IsNNS() {
...
func parseContexts(cmd *cobra.Command) ([]session.Context, error) {
	...
	if len(objStrs) > 0 {
		...
		err = ctx.SetObjects(objIDs)
		if err != nil {
			return nil, fmt.Errorf("can't set objects: %w", err)
		}
	}

	return []session.Context{ctx}, nil
}
