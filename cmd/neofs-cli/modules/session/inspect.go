...
func displayTokenV2(tok *session.TokenV2) error {
	...
	issuer := tok.Issuer()
	if issuer.IsOwnerID() {
		fmt.Printf("Issuer: %s (UserID)\n", issuer.OwnerID().String())
	} else if issuer.IsNNS() {
		fmt.Printf("Issuer: %s (NNS)\n", issuer.NNSName())
	}
	...
	if len(chain) > 0 {
		...
		for i, delegation := range chain {
			...
			for j, subj := range subjects {
				if subj.IsOwnerID() {
					fmt.Printf("\t\t\t%d. %s (UserID)\n", j+1, subj.OwnerID().String())
				} else if subj.IsNNS() {
					fmt.Printf("\t\t\t%d. %s (NNS)\n", j+1, subj.NNSName())
				}
			}

			fmt.Printf("\t\tLifetime: iat=%d, nbf=%d, exp=%d\n", delegation.Iat(), delegation.Nbf(), delegation.Exp())

			verbs := delegation.Verbs()
			fmt.Printf("\t\tVerbs: ")
			for j, verb := range verbs {
				if j > 0 {
					fmt.Print(", ")
	...
	if err := tok.ValidateDelegationChain(); err != nil {
		fmt.Printf("\tDelegation Chain: INVALID (%v)\n", err)
	} else if len(chain) > 0 {
		fmt.Printf("\tDelegation Chain: VALID\n")
	}
...
func verbV2ToString(verb session.VerbV2) string {
	switch verb {
	case session.VerbV2ObjectGet:
		return "GET"
	case session.VerbV2ObjectPut:
		return "PUT"
	case session.VerbV2ObjectHead:
		return "HEAD"
	case session.VerbV2ObjectSearch:
		return "SEARCH"
	case session.VerbV2ObjectDelete:
		return "DELETE"
	case session.VerbV2ObjectRange:
		return "RANGE"
	case session.VerbV2ObjectRangeHash:
		return "RANGEHASH"
	case session.VerbV2ContainerPut:
		return "CONTAINER_PUT"
	case session.VerbV2ContainerDelete:
		return "CONTAINER_DELETE"
	case session.VerbV2ContainerSetEACL:
