...
func initContainerService(c *cfg) {
	...
	// TODO: report more that once in every epoch?
	addNewEpochAsyncNotificationHandler(c, func(e event.Event) {
		...
		l := c.log.With(zap.String("component", "container_reports"), zap.Uint64("epoch", ev.EpochNumber()))

		l.Debug("sending container report to contract...", zap.Uint64("epoch", ev.EpochNumber()))
		...
		for _, cnr := range idList {
			size, objsNum, err := st.ContainerSize(cnr)
			if err != nil {
				l.Warn("container's stat fetching error",
			...
			l.Debug("successfully put container report to contract",
				zap.Stringer("cid", cnr), zap.Uint64("size", size), zap.Uint64("objects_num", objsNum))
		}

		l.Debug("successfully sent all container reports",
			zap.Int("num_of_reports", len(idList)), zap.Uint64("epoch", ev.EpochNumber()))
	})
