...
func (c *usedSpaceService) makeResponse(body *protocontainer.AnnounceUsedSpaceResponse_Body, code uint32, msg string) (*protocontainer.AnnounceUsedSpaceResponse, error) {
	v := version.Current()
...
func (c *usedSpaceService) makeFailedResponse(code uint32, err error) (*protocontainer.AnnounceUsedSpaceResponse, error) {
	return c.makeResponse(nil, code, err.Error())
}
...
func (c *usedSpaceService) AnnounceUsedSpace(ctx context.Context, req *protocontainer.AnnounceUsedSpaceRequest) (*protocontainer.AnnounceUsedSpaceResponse, error) {
	...
	if err := signature.VerifyServiceMessage(putReq); err != nil {
		return c.makeFailedResponse(1026, err)
	}
	...
	if err != nil {
		return c.makeFailedResponse(1024, fmt.Errorf("could not initialize container's used space writer: %w", err))
	}
	...
	for _, a := range req.GetBody().GetAnnouncements() {
		...
		if err != nil {
			return c.makeFailedResponse(1024, fmt.Errorf("invalid size announcement: %w", err))
		}

		if err := c.processLoadValue(ctx, est, passedRoute, w); err != nil {
			return c.makeFailedResponse(1024, err)
		}
	}

	return c.makeResponse(nil, 0, "")
}
