...
type InitialTrustSource struct {
	NetMap netmapcore.Source
}

var ErrEmptyNetMap = errors.New("empty NepMap")

// InitialTrust returns `initialTrust` as an initial trust value.
func (i InitialTrustSource) InitialTrust(reputation.PeerID) (localreputation.TrustValue, error) {
	epoch, err := i.NetMap.Epoch()
	if err != nil {
		return localreputation.TrustZero, fmt.Errorf("failed to get epoch: %w", err)
	}
	epoch = max(1, epoch) // prevent underflow below
	nm, err := i.NetMap.GetNetMapByEpoch(epoch - 1)
	if err != nil {
		return localreputation.TrustZero, fmt.Errorf("failed to get NetMap: %w", err)
	}

	nodeCount := localreputation.TrustValueFromFloat64(float64(len(nm.Nodes())))
	if nodeCount == 0 {
		return localreputation.TrustZero, ErrEmptyNetMap
	}

	return localreputation.TrustOne.Div(nodeCount), nil
}
