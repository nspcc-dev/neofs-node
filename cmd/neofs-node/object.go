...
type putPostPlacementReplicator struct {
	log        *zap.Logger
	replicator *replicator.Replicator
}

func (r putPostPlacementReplicator) HandlePostPlacement(ctx context.Context, obj *object.Object, nodes []netmapsdk.NodeInfo) {
	if len(nodes) == 0 {
		return
	}
	...
	if err := r.replicator.EnqueueTask(ctx, task); err != nil {
		r.log.Debug("could not enqueue post-placement replication",
...
func initObjectService(c *cfg) {
	...
	*c.cfgObject.getSvc = *sGet // need smth better
	...
	sPut := putsvc.NewService(&transport{clients: putConstructor}, c, c.metaService,
		...
		putsvc.WithPostPlacementReplicator(putPostPlacementReplicator{log: c.log, replicator: c.replicator}),
