...
func (s *reputationServer) makeResponseMetaHeader(code uint32, msg string) *protosession.ResponseMetaHeader {
	v := version.Current()
...
func (s *reputationServer) makeLocalResponse(code uint32, err error) (*protoreputation.AnnounceLocalTrustResponse, error) {
	var msg string
	if err != nil {
		msg = err.Error()
	}
...
func (s *reputationServer) AnnounceLocalTrust(ctx context.Context, req *protoreputation.AnnounceLocalTrustRequest) (*protoreputation.AnnounceLocalTrustResponse, error) {
	...
	if err := signature.VerifyServiceMessage(req2); err != nil {
		return s.makeLocalResponse(1026, err)
	}
	...
	if err != nil {
		return s.makeLocalResponse(1024, fmt.Errorf("could not initialize local trust writer: %w", err))
	}
	...
	for _, trust := range body.GetTrusts() {
		...
		if err != nil {
			return s.makeLocalResponse(1024, fmt.Errorf("could not write one of local trusts: %w", err))
		}
	}

	return s.makeLocalResponse(0, nil)
}

func (s *reputationServer) makeIntermediateResponse(code uint32, err error) (*protoreputation.AnnounceIntermediateResultResponse, error) {
	var msg string
	if err != nil {
		msg = err.Error()
	}
...
func (s *reputationServer) AnnounceIntermediateResult(ctx context.Context, req *protoreputation.AnnounceIntermediateResultRequest) (*protoreputation.AnnounceIntermediateResultResponse, error) {
	...
	// TODO: use const codes on SDK upgrade
	if err := signature.VerifyServiceMessage(req2); err != nil {
		return s.makeIntermediateResponse(1026, err)
	}
	...
	if err != nil {
		return s.makeIntermediateResponse(1026, fmt.Errorf("could not initialize trust writer: %w", err))
	}
	...
	err = w.Write(trust)
	if err != nil {
		return s.makeIntermediateResponse(1026, fmt.Errorf("could not write trust: %w", err))
	}

	return s.makeIntermediateResponse(0, nil)
}
