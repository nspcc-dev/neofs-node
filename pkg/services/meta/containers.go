...
func (s *containerStorage) putRawIndexes(ctx context.Context, ee []objEvent, net NeoFSNetwork) error {
	...
	for i, e := range ee {
		...
		if len(e.deletedObjects) > 0 {
			...
			deleteObjectsOps(batch, s.mptOpsBatch, s.db, e.deletedObjects)
		}
		if len(e.lockedObjects) > 0 {
...
func deleteObjectsOps(dbKV, mptKV map[string][]byte, s storage.Store, objects []byte) {
	...
	for len(objects) > 0 {
		...
		s.Seek(rng, func(k, v []byte) bool {
			...
			case oidToAttrIndex:
				...
				attrK := k[1+oid.Size : i]
				attrV := k[i+attributeDelimiterLen:]
