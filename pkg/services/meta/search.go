...
func (s *containerStorage) search(fs object.SearchFilters, fInt map[int]objectcore.ParsedIntFilter, attrs []string, cursor *objectcore.SearchCursor, count uint16) ([]client.SearchResultItem, []byte, error) {
	...
	if cursor != nil {
		rng.Start = cursor.PrimarySeekKey
	}
	s.db.Seek(rng, func(k, v []byte) bool {
		if cursor != nil && bytes.Equal(v, cursor.PrimarySeekKey) {
...
func searchUnfiltered(st storage.Store, cursor *objectcore.SearchCursor, count uint16) ([]client.SearchResultItem, []byte, error) {
	...
	st.Seek(rng, func(k, v []byte) bool {
...
func (a *attrGetter) Get(oID []byte, attributeKey string) (attributeValue []byte, err error) {
	...
	a.keyBuff = append(a.keyBuff, objectcore.AttributeDelimiter...)
	...
	a.db.Seek(rng, func(k, v []byte) bool {
		...
		attributeValue = k[len(a.keyBuff):]
