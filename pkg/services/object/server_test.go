...
func TestServer_Replicate(t *testing.T) {
	...
	noCallSrv := objectSvc.New(noCallObjSvc, 0, &noCallFSChain, noCallStorage, neofscryptotest.Signer().ECDSAPrivateKey, nopMetrics{})
	...
	t.Run("apply storage policy failure", func(t *testing.T) {
		...
		srv := objectSvc.New(noCallObjSvc, 0, fsChain, noCallStorage, neofscryptotest.Signer().ECDSAPrivateKey, nopMetrics{})
	...
	t.Run("local storage failure", func(t *testing.T) {
		...
		srv := objectSvc.New(noCallObjSvc, 0, fsChain, s, neofscryptotest.Signer().ECDSAPrivateKey, nopMetrics{})
	...
	t.Run("meta information signature", func(t *testing.T) {
		...
		srv := objectSvc.New(noCallObjSvc, mNumber, fsChain, s, signer.ECDSAPrivateKey, nopMetrics{})
...
func BenchmarkServer_Replicate(b *testing.B) {
	...
	srv := objectSvc.New(nil, 0, fsChain, nopStorage{}, neofscryptotest.Signer().ECDSAPrivateKey, nopMetrics{})
