...
func (t *distributedTarget) Close() (oid.ID, error) {
	...
	expectedVUB := (uint64(t.currentBlock)/t.currentEpochDuration + 1) * t.currentEpochDuration
	...
	if t.localNodeInContainer && !node.local {
		...
		for i, sig := range sigs {
			...
			if !sig.Verify(t.objSharedMeta) {
				l.Info("metadata: signature verification failed", zap.Int("signature index", i))
				continue
			}
...
func decodeSignatures(b []byte) ([]neofscrypto.Signature, error) {
	res := make([]neofscrypto.Signature, 2)
	for i := range res {
		var offset int
		var err error
