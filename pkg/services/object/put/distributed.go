...
type distributedTarget struct {
	...
	// When object from request is an EC part, ruleIdx is >= 0.
...
func (t *distributedTarget) saveObject(obj objectSDK.Object, objMeta object.ContentMeta, encObj encodedObject) error {
	...
	if ecRules := t.containerNodes.ECRules(); len(ecRules) > 0 {
		if t.ecPart.ruleIdx >= 0 { // already encoded EC part
			total := int(ecRules[t.ecPart.ruleIdx].DataParts + ecRules[t.ecPart.ruleIdx].ParityParts)
			nodes := objNodeLists[len(repRules)+t.ecPart.ruleIdx]
			return t.saveECPart(obj, objMeta, encObj, t.ecPart.partIdx, total, nodes)
		}
