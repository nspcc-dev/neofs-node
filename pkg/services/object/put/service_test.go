...
func Test_Slicing_EC(t *testing.T) {
	rules := []ECRule{
		{DataParts: 2, ParityParts: 2},
		{DataParts: 3, ParityParts: 1},
		{DataParts: 6, ParityParts: 3},
		{DataParts: 12, ParityParts: 4},
	}
...
func testSlicingECRules(t *testing.T, ln uint64, rules []ECRule) {
	maxRule := slices.MaxFunc(rules, func(a, b ECRule) int {
		return cmp.Compare(a.DataParts+a.ParityParts, b.DataParts+b.ParityParts)
	})

	maxTotalParts := int(maxRule.DataParts + maxRule.ParityParts)
	const cnrReserveNodes = 2
	const outCnrNodes = 2
	...
	testThroughNode := func(t *testing.T, idx int) {
		...
		require.Zero(t, twoDimSliceElementCount(nodeObjLists))
...
func (x mockContainerNodes) ECRules() []ECRule {
	return x.ecRules
}
...
func checkAndGetObjectFromECParts(t *testing.T, limit uint64, rule ECRule, parts []object.Object) object.Object {
	require.Len(t, parts, int(rule.DataParts+rule.ParityParts))
...
func checkAndGetPayloadFromECParts(t *testing.T, ln uint64, rule ECRule, parts []object.Object) []byte {
	...
	for lostCount := 1; lostCount <= int(rule.ParityParts); lostCount++ {
		for lostIdxs := range combos(len(payloadParts), lostCount) {
			brokenParts := excludeTwoDimSliceElements(payloadParts, lostIdxs)
			...
			require.Equal(t, payloadParts[:rule.DataParts], brokenParts[:rule.DataParts])
		}
	}

	for lostIdxs := range combos(len(payloadParts), int(rule.ParityParts)+1) {
		require.Error(t, enc.Reconstruct(excludeTwoDimSliceElements(payloadParts, lostIdxs)))
		require.Error(t, enc.ReconstructSome(excludeTwoDimSliceElements(payloadParts, lostIdxs), required))
	}

	payload := slices.Concat(payloadParts[:rule.DataParts]...)
...
func checkAndGetECPartInfo(t testing.TB, part object.Object) (int, int) {
	...
	ruleIdxAttr, _ := getAttribute(attrs, "__NEOFS__EC_RULE_IDX")
	require.NotZero(t, ruleIdxAttr)
	ruleIdx, err := strconv.Atoi(ruleIdxAttr)
	require.NoError(t, err)
	require.True(t, ruleIdx >= 0)

	partIdxAttr, _ := getAttribute(attrs, "__NEOFS__EC_PART_IDX")
...
func checkAndCutSplitECObject(t *testing.T, ln uint64, sessionToken session.Object, rules []ECRule, nodeObjLists [][]object.Object) object.Object {
	splitPartCount := splitMembersCount(maxObjectSize, ln)

	var expectedCount int
	for i := range rules {
		expectedCount += int(rules[i].DataParts+rules[i].ParityParts) * splitPartCount
	}

	require.EqualValues(t, expectedCount, twoDimSliceElementCount(nodeObjLists))
...
func checkAndCutUnsplitECObject(t *testing.T, rules []ECRule, nodeObjLists [][]object.Object) object.Object {
...
func checkAndCutECPartsForRule(t *testing.T, ruleIdx int, rule ECRule, nodeObjLists [][]object.Object) []object.Object {
	var parts []object.Object

	for i := range rule.DataParts + rule.ParityParts {
...
// TODO: worth sharing across repository.
func twoDimSliceElementCount[E any](s [][]E) (n int) {
	for i := range s {
		n += len(s[i])
	}
	return
...
func getAttribute(s []object.Attribute, e string) (v string, ind int) {
	ind = slices.IndexFunc(s, func(attr object.Attribute) bool { return attr.Key() == e })
	if ind >= 0 {
		v = s[ind].Value()
	}
	return
...
func excludeTwoDimSliceElements[T any, S []T, SS []S](ss SS, idxs []int) SS {
	cp := slices.Clone(ss)
	for i, part := range cp {
		cp[i] = slices.Clone(part)
	}
