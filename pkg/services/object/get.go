...
func (x *getECTransport) CopyECParentHeaderAndPayloadFromLocalFirstPart(ctx context.Context, storage *engine.StorageEngine, ruleIdx int) (bool, uint64, uint64, uint64, error) {
	...
	prefixLen, stream, err := storage.ReadECPart(ctx, x.requestContainer, x.requestObject, partInfo, blobstor.PayloadRange{}, buf, nil)
	if err != nil {
...
func (x *getECTransport) CopyLocalECPartRange(ctx context.Context, storage *engine.StorageEngine, partInfo iec.PartInfo, off, ln uint64, controlCh <-chan bool) (uint64, error) {
	...
	stream, err := storage.ReadECPartRange(ctx, x.requestContainer, x.requestObject, partInfo, off, ln, buf, nil)
	if err != nil {
...
func (x *getECTransport) copyRemotePartRangeWithRequest(ctx context.Context, conn *grpc.ClientConn, request *[]byte, ln uint64, controlCh <-chan bool) (uint64, error) {
	...
	for {
		...
		if copied+uint64(chunkLen) > ln {
			return 0, fmt.Errorf("received more bytes than requested: expected %d, already copied %d, current chunk %d", ln, copied, chunkLen)
		}
