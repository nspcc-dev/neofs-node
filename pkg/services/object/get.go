...
func (x *getProxyContext) handleGetResponse(streamProg *getStreamProgress, respBuf mem.BufferSlice) (bool, bool, error) {
	if len(respBuf) == 0 {
		return false, false, errors.New("empty response")
	}

	var localRespBuf *iprotobuf.MemBuffer
	var buf []byte
	if len(respBuf) == 1 {
		buf = respBuf[0].ReadOnlyData()
	} else if ln := respBuf.Len(); ln <= maxGetResponseChunkLen {
		if ln <= 2*iobject.NonPayloadFieldsBufferLength {
			localRespBuf, buf = getBufferForHeadResponse()
		} else {
			localRespBuf, buf = getBufferForChunkGetResponse()
		}
		defer localRespBuf.Free()
		buf = buf[:respBuf.CopyTo(buf)]
	} else {
	...
	sent, err := x.handleResponseBody(streamProg, respBuf, body, localRespBuf)
	if err != nil {
...
func (x *getProxyContext) handleResponseBody(streamProg *getStreamProgress, respBuf mem.BufferSlice, buf []byte, localRespBuf *iprotobuf.MemBuffer) (bool, error) {
	var oneofNum protowire.Number
	var oneofFld []byte
	var opts protoscan.ScanMessageOptions

	opts.InterceptBytes = func(num protowire.Number, b []byte) error {
	...
	err := protoscan.ScanMessage(buf, protoscan.ObjectGetResponseBodyScheme, opts)
	if err != nil {
		return false, err
	}
	...
	case protoobject.FieldGetResponseBodyChunk:
		return x.handleChunkResponse(streamProg, respBuf, oneofFld, localRespBuf)
	case protoobject.FieldGetResponseBodySplitInfo:
		return x.handleSplitInfo(respBuf, oneofFld)
	}
}

func (x *getProxyContext) handleInitResponse(respBuf mem.BufferSlice, buf []byte) (bool, error) {
	var hdr []byte
	var opts protoscan.ScanMessageOptions
	...
	opts.InterceptNested = func(num protowire.Number, fld []byte) error {
		...
		opts.InterceptNested = func(num protowire.Number, b []byte, checkOrder bool) (bool, error) {
			...
			opts.InterceptBytes = func(num protowire.Number, b []byte) error {
				if num == protorefs.FieldChecksumValue {
					x.payloadHashCheck = b
				}
				return nil
			}
			return protoscan.ScanMessageOrdered(b, protoscan.ChecksumScheme, opts)
		}
		...
		if err = checkHeaderProtobufAgainstID(fld, x.reqOID, hdrOrdered); err != nil {
			return err
		}
	...
	err := protoscan.ScanMessage(buf, protoscan.ObjectGetResponseInitScheme, opts)
	if err != nil {
		return false, err
	}
...
func (x *getProxyContext) handleChunkResponse(streamProg *getStreamProgress, respBuf mem.BufferSlice, chunk []byte, localRespBuf *iprotobuf.MemBuffer) (bool, error) {
	respChunk := chunkToSend(x.respondedPayload, streamProg.readPayload, chunk)
	if len(respChunk) == 0 {
		streamProg.readPayload += len(chunk)
		return false, nil
	}
	...
	remoteSent := len(respChunk) == len(chunk)
	if !remoteSent {
		if localRespBuf != nil {
			copy(localRespBuf.SliceBuffer[maxChunkOffsetInGetResponse:], respChunk)

			bodyf := shiftPayloadChunkInGetResponseBuffer(localRespBuf.SliceBuffer, maxChunkOffsetInGetResponse, len(respChunk))
			...
			// FIXME: alloc
			bodyFldLen := 1 + protowire.SizeBytes(len(respChunk))
			...
			off += 1 + binary.PutUvarint(b[off+1:], uint64(len(respChunk)))
			off += copy(b[off:], respChunk)
			if x.respStream.signResponse {
...
func (x *getProxyContext) handleSplitInfo(respBuf mem.BufferSlice, buf []byte) (bool, error) {
	...
	if compose {
		...
		opts.InterceptNested = func(num protowire.Number, fld []byte) error {
			...
			var opts protoscan.ScanMessageOptions
			opts.InterceptBytes = func(num2 protowire.Number, b []byte) error {
				if num2 == protorefs.FieldObjectIDValue {
					switch num {
					case protoobject.FieldSplitInfoLastPart:
						si.SetLastPart(oid.ID(b))
			...
			return protoscan.ScanMessage(fld, protoscan.ObjectIDScheme, opts)
		}
	}

	err := protoscan.ScanMessage(buf, protoscan.ObjectSplitInfoScheme, opts)
	if err != nil {
