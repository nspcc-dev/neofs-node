	...
	if !commonPrm.LocalOnly() {
		...
		p.SetRequestForwarder(groupAddressRequestForwarder(func(ctx context.Context, addr network.Address, c client.MultiAddressClient, pubkey []byte) (*object.Object, error) {
			...
			err = c.RawForAddress(addr, func(conn *grpc.ClientConn) error {
				...
				getStream, err = protoobject.NewObjectServiceClient(conn).Get(stream.Context(), req.ToGRPCMessage().(*protoobject.GetRequest))
				return err
			})
			if err != nil {
...
func (s *Service) toRangePrm(req *objectV2.GetRangeRequest, stream objectSvc.GetObjectRangeStream) (*getsvc.RangePrm, error) {
	...
	if !commonPrm.LocalOnly() {
		...
		p.SetRequestForwarder(groupAddressRequestForwarder(func(ctx context.Context, addr network.Address, c client.MultiAddressClient, pubkey []byte) (*object.Object, error) {
			...
			err = c.RawForAddress(addr, func(conn *grpc.ClientConn) error {
				// FIXME: context should be cancelled on return from upper func
				rangeStream, err = protoobject.NewObjectServiceClient(conn).GetRange(stream.Context(), req.ToGRPCMessage().(*protoobject.GetRangeRequest))
				return err
			})
			if err != nil {
