...
func (s *Service) restoreFromECParts(ctx context.Context, cnr cid.ID, id oid.ID, ecRules []iec.Rule, sortedNodeLists [][]netmap.NodeInfo) (object.Object, error) {
	...
	for i := range ecRules {
		...
		s.log.Info("failed to restore object by EC rule", zap.Stringer("rule", ecRules[i]), zap.Int("ruleIdx", i), zap.Error(err)) // TODO: more fields
	}
...
func (s *Service) restoreFromECPartsByRule(ctx context.Context, cnr cid.ID, parent oid.ID, rule iec.Rule, ruleIdx int, sortedNodes []netmap.NodeInfo) (object.Object, error) {
	...
	// TODO: check node access order is the same for PUT and GET
	// FIXME: if one of the servers hangs, it eats up the entire context
	// FIXME: if 'already removed' abort
	for i := range int(rule.DataPartNum) {
		wg.Add(1)
		go func(partIdx int) {
			defer wg.Done()

			par, part, err := s.getECPart(ctx, cnr, parent, rule, ruleIdx, sortedNodes, partIdx)
			if err != nil {
	...
	if rem == 0 {
		...
		hdr.SetPayload(slices.Concat(parts[:rule.DataPartNum]...)[:hdr.PayloadSize()])
		return hdr, nil
	}
	...
	for i := range rule.ParityPartNum {
		...
		go func(partIdx int) {
			...
			_, pldPart, err := s.getECPart(ctx, cnr, parent, rule, ruleIdx, sortedNodes, partIdx)
			if err != nil {
...
// Returns parent header and part payload.
//
// Returns [apistatus.ErrObjectAlreadyRemoved] if object was marked for removal.
func (s *Service) getECPart(ctx context.Context, cnr cid.ID, parent oid.ID, rule iec.Rule, ruleIdx int, sortedNodes []netmap.NodeInfo, partIdx int) (object.Object, []byte, error) {
	...
	for i := range iec.NodeSequenceForPartReading(partIdx, int(rule.DataPartNum+rule.ParityPartNum), len(sortedNodes)) {
		...
		if local {
			...
			part, err = s.getECPartFromRemoteNode(ctx, cnr, parent, pi, sortedNodes[i])
		}
		if err == nil {
			break
		}
		...
		if !errors.Is(err, apistatus.ErrObjectNotFound) {
			s.log.Info("failed to get EC part from node, continue...", zap.Bool("local", local), zap.Stringer("container", cnr),
				zap.Stringer("parent", parent), zap.Error(err)) // if error from remote SN, it contains net address
		}
	}
	if err != nil {
...
// Returns [apistatus.ErrObjectNotFound] if object is missing on the node.
// Returns [apistatus.ErrObjectAlreadyRemoved] if object was marked for removal.
func (s *Service) getECPartFromRemoteNode(ctx context.Context, cnr cid.ID, parent oid.ID, pi iec.PartInfo, node netmap.NodeInfo) (object.Object, error) {
