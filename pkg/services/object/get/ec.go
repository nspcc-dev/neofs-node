...
func (s *Service) restoreFromECPartsByRule(ctx context.Context, cnr cid.ID, id oid.ID, rule iec.Rule, ruleIdx int, sortedNodes []netmap.NodeInfo) (object.Object, error) {
	...
	for i := range int(rule.DataPartNum) {
		...
		go func(partIdx int) {
			...
			// TODO: explore streaming options. See reedsolomon.NewStream. Cmp performance
...
func (s *Service) getECPart(ctx context.Context, cnr cid.ID, parent oid.ID, rule iec.Rule, ruleIdx int, sortedNodes []netmap.NodeInfo, partIdx int) (object.Object, []byte, error) {
	...
	for i := range iec.NodeSequenceForPart(partIdx, int(rule.DataPartNum+rule.ParityPartNum), len(sortedNodes)) {
		...
		if local {
			...
			hdr, rc, err = s.getECPartFromNode(ctx, cnr, parent, pi, sortedNodes[i])
		}
		if err == nil {
			break
		}
...
func (s *Service) getECPartFromNode(ctx context.Context, cnr cid.ID, parent oid.ID, pi iec.PartInfo, node netmap.NodeInfo) (object.Object, io.ReadCloser, error) {
	...
	if err != nil {
		return object.Object{}, nil, fmt.Errorf("get connection to remote SN: %w", err)
	}
	...
	// FIXME: access tokens
	hdr, rc, err := conn.ObjectGetInit(ctx, cnr, parent, user.NewAutoIDSigner(*localNodeKey), opts)
	if err != nil {
		return object.Object{}, nil, fmt.Errorf("get object API (header): %w", err)
	}
