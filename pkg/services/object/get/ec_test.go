...
func TestService_Get_EC(t *testing.T) {
	...
	nodeSvcs := make([]delayedService, rule.DataPartNum+rule.ParityPartNum)
	...
	tc := &testECServiceConn{
		...
		sTok:  &sTok,
		bTok:  &bTok,
		nodes: make(map[string]*delayedService),
	}
	for i := range nodeSvcs {
	...
	t.Run("already removed", func(t *testing.T) {
		t.Run("parity part", func(t *testing.T) {
			// All data parts' carriers respond OK with delay D except one who respond 404.
			// All parity parts' carriers respond OK with delay D except one who respond 'already removed' with delay d<<D.
			// Since 'already removed' status must abort the op, we expect receiving it in D+d+epsilon.
			const failDelay = 50 * time.Millisecond
			for i := range nodeSvcs {
				...
				if i == int(rule.DataPartNum-1) {
					localVal.err = fmt.Errorf("some context: %w", apistatus.ErrObjectNotFound)
					delay = failDelay
				} else if i == int(rule.DataPartNum) {
					localVal.err = fmt.Errorf("some context: %w", apistatus.ErrObjectAlreadyRemoved)
					delay = failDelay
				}

				nodeSvcs[i].delay = delay
				nodeSvcs[i].svc.localObjects = &mockLocalObjects{
					getECPart: map[getECPartKey]getECPartValue{
			...
			// FIXME
			require.WithinDuration(t, timeBefore, timeAfter, failDelay+failDelay*11/10)
		})

		// All data parts' carriers respond OK with delay D except one who respond 'already removed' with delay d<<D.
		// Since 'already removed' status must abort the op, we expect receiving it in d+epsilon.
		const removedDelay = 50 * time.Millisecond
		const othersDelay = 10 * removedDelay
		for i := range nodeSvcs {
			localVal := getECPartValue{
				hdr: *partObjs[i].CutPayload(),
				rdr: io.NopCloser(bytes.NewReader(partObjs[i].Payload())),
		...
		require.ErrorIs(t, err, apistatus.ErrObjectAlreadyRemoved)
		require.WithinDuration(t, timeBefore, timeAfter, removedDelay*11/10)
	})
...
type testECServiceConn struct {
	...
	nodes map[string]*delayedService
}
...
func (x *testECServiceConn) InitGetObjectStream(ctx context.Context, node netmap.NodeInfo, pk ecdsa.PrivateKey,
	...
	if err := v.svc.Get(ctx, prm); err != nil {
