...
func (s *Service) Get(ctx context.Context, prm Prm) error {
	...
	if pi.RuleIndex >= 0 {
		...
		return s.copyLocalECPart(prm.addr.Container(), prm.addr.Object(), pi, prm.objWriter)
	}

	if len(repRules) > 0 { // REP format does not require encoding
		// TODO: reuse nodesLists
		err := s.get(ctx, prm.commonPrm).err
		if len(ecRules) == 0 || !errors.Is(err, apistatus.ErrObjectNotFound) {
			return err
		}
	}

	return s.copyECObject(ctx, prm.addr.Container(), prm.addr.Object(), ecRules, nodeLists, prm.objWriter)
}
