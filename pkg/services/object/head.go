...
func getHeaderFromRemoteNode(ctx context.Context, conn *grpc.ClientConn, req *protoobject.HeadRequest, reqOID oid.ID) (mem.BufferSlice, iprotobuf.BuffersSlice, error) {
	...
	hdrBuffers, err := handleHeadResponse(respBuf, reqOID)
	if err != nil {
		respBuf.Free()
...
func handleHeadResponse(respBuf mem.BufferSlice, reqOID oid.ID) (iprotobuf.BuffersSlice, error) {
	var code uint32
	...
	hdrBuffers, err := handleHeadResponseBody(body, reqOID)
	if err != nil {
...
func handleHeadResponseBody(buffers iprotobuf.BuffersSlice, reqOID oid.ID) (iprotobuf.BuffersSlice, error) {
	...
	case protoobject.FieldHeadResponseBodyHeader:
		hdrBuffers, err := handleHeaderWithSignature(oneofFld, reqOID)
		if err != nil {
...
func handleHeaderWithSignature(buffers iprotobuf.BuffersSlice, reqOID oid.ID) (iprotobuf.BuffersSlice, error) {
	var withHdr bool
