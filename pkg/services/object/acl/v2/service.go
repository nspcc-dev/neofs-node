...
func (b Service) decodeAndVerifySessionTokenV2Common(m *protosession.SessionTokenV2) (sessionSDKv2.Token, error) {
	...
	hasUserInNNS := func(name string, userID user.ID) bool {
		ok, err := b.c.fsChain.HasUserInNNS(name, userID.ScriptHash())
		if err != nil {
	...
	if b.nodeKey != nil {
		if !token.AssertAuthority(user.NewFromECDSAPublicKey(*b.nodeKey), hasUserInNNS) {
			if authErr != nil {
				return token, authErr
			}
	...
	if !token.ValidAt(currentTime) {
		return token, fmt.Errorf("%s: V2 token is invalid at %d Unix time", invalidRequestMessage, currentTime)
	}
