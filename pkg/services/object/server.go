...
func New(hs Handlers, fsChain FSChain, st Storage, metaSvc *metasvc.Meta, signer ecdsa.PrivateKey, m MetricCollector, ac aclsvc.ACLChecker, rp ACLInfoExtractor, cs ClientConstructor, log *zap.Logger) *Server {
...
func (s *Server) Get(req *protoobject.GetRequest, gStream protoobject.ObjectService_GetServer) error {
	...
	p.WithBuffer(hdrBuf, checkHeaderBinaryFn, func(ln int, s io.ReadCloser) { hdrLen, stream = ln, s })
	...
	isPartialRange := p.HasRange() && !p.IsFullRange()
	payloadOnly := p.PayloadOnly()

	var pldFldOff int
	if !isPartialRange || !payloadOnly {
...
func sendECPartIdxInResponse(req *protoobject.GetRequest) bool {
	if req.MetaHeader == nil || !req.Body.PayloadOnly || req.Body.Range != nil {
		return false
	}
	var (
		isECPartReq  bool
...
func (s *Server) ProcessSearch(ctx context.Context, req *protoobject.SearchV2Request, localOnly bool, forwardedResponseToError bool, cID cid.ID) ([]client.SearchResultItem, []byte, error) {
	...
	default:
		...
		getRequestForConn := func(conn clientcore.MultiAddressClient) ([]byte, error) {
			v := conn.APIVersion()
			ver := version.New(v.GetMajor(), v.GetMinor())

			if ver.Compare(localVersion) >= 0 {
				if clientcore.IsMutuallyAuthenticated(conn) { // unsigned request
					return reqBuf[:bodyWithMetaLen], nil
				}
				// 1 signature
				onceResign.Do(func() {
					signingErr = s.writeRequestSignatures(reqBuf, bodyWithMetaLen, bodySlice, metaHdrSlice, localVersion)
				})
				if signingErr != nil {
					return nil, fmt.Errorf("sign request: %w", signingErr)
				}
				return reqBuf, nil
			}
		...
		iterateSearchableContainerNodes(nodeSets, repRules, ecRules, !optimizedNodes, func(node netmap.NodeInfo) bool {
			...
			go func() {
				set, more, err := s.searchOnRemoteNode(ctx, node, req.Body, getRequestForConn)
...
func (s *Server) searchOnRemoteNode(ctx context.Context, node netmap.NodeInfo, reqBody *protoobject.SearchV2Request_Body, getRequestForConn func(clientcore.MultiAddressClient) ([]byte, error)) ([]client.SearchResultItem, bool, error) {
	c, err := s.nodeClients.Get(ctx, node)
	if err != nil {
		return nil, false, fmt.Errorf("get node client: %w", err)
	}

	req, err := getRequestForConn(c)
	if err != nil {
	...
	return items, more, c.ForAnyGRPCConn(ctx, func(ctx context.Context, conn *grpc.ClientConn) error {
		...
		if err != nil {
			s.log.Error("failed to search objects on remote node", zap.Error(err))
		}
		return err
	})
}
