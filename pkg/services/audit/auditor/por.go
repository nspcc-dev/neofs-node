...
func (c *Context) checkStorageGroupPoR(ind int, sg *oidSDK.ID) {
	...
	homomorphicHashingEnabled := c.task.ContainerStructure().HomomorphicHashingState()
	...
	tzCheck := !homomorphicHashingEnabled || bytes.Equal(tzHash, storageGroup.ValidationDataHash().Sum())
