...
type checkECPartsTask struct {
	container   cid.ID
	parent      oid.ID
	rule        iec.Rule
	localPartID oid.ID
	localPart   iec.PartInfo
}

func (p *Policer) listenCheckECPartsTasks(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		case task := <-p.checkECPartsTaskCh:
			err := p.checkECPartsWorkerPool.Submit(func() {
				p.checkECParts(ctx, task.container, task.parent, task.rule, task.localPart.RuleIndex, task.localPart.Index, task.localPartID)
			})
			if errors.Is(err, ants.ErrPoolOverload) {
				p.log.Info("pool of workers for EC parts checking is full, skip the task",
					zap.Stringer("container", task.container), zap.Stringer("parent", task.parent))
			} else if err != nil {
...
func (p *Policer) tryScheduleCheckECPartsTask(ctx context.Context, cnr cid.ID, parent oid.ID, rule iec.Rule, localPartID oid.ID, localPartInfo iec.PartInfo) {
	select {
	case <-ctx.Done():
	case p.checkECPartsTaskCh <- checkECPartsTask{
		container:   cnr,
		parent:      parent,
		rule:        rule,
		localPartID: localPartID,
		localPart:   localPartInfo,
	}:
	}
}
...
getNextPart:
	...
	p.tryScheduleRecreateECPartsTask(ctx, parentHdr, rule, ruleIdx, parts, missingIdx)
}
...
type recreateECPartsTask struct {
...
// TODO: call from GET service.
// TODO: docs.
func (p *Policer) TryScheduleRecreateECPartsTask(ctx context.Context, parent object.Object, rule iec.Rule, ruleIdx int, parts [][]byte) {
	missingIdx := make([]int, 0, rule.ParityPartNum)
	for i := range parts {
		if parts[i] != nil {
			continue
		}

		if len(missingIdx) == int(rule.ParityPartNum) { // should never happen if task sender counts the parts correctly
			p.log.Error("too many missing EC parts submitted for recreation",
	...
	p.tryScheduleRecreateECPartsTask(ctx, parent, rule, ruleIdx, parts, missingIdx)
}

func (p *Policer) tryScheduleRecreateECPartsTask(ctx context.Context, parent object.Object, rule iec.Rule, ruleIdx int, parts [][]byte, missingIdx []int) {
	select {
	case <-ctx.Done():
	case p.recreateECPartsTaskCh <- recreateECPartsTask{
		parent:     parent,
		rule:       rule,
		ruleIdx:    ruleIdx,
		parts:      parts,
...
func (p *Policer) listenRecreateECPartsTasks(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		case task := <-p.recreateECPartsTaskCh:
			err := p.recreateECPartsWorkerPool.Submit(func() {
				p.recreateECParts(ctx, task.parent, task.rule, task.ruleIdx, task.parts, task.missingIdx)
			})
			if errors.Is(err, ants.ErrPoolOverload) {
				p.log.Info("pool of workers for EC part recreation is full, skip the task",
					zap.Stringer("container", task.parent.GetContainerID()), zap.Stringer("parent", task.parent.GetID()))
			} else if err != nil {
				p.log.Warn("unexpected error returned from pool of workers for EC part recreation", zap.Error(err))
			}
		}
	}
}
...
func (p *Policer) recreateECPart(ctx context.Context, parent object.Object, rule iec.Rule, ruleIdx, partIdx int, part []byte, sortedNodes []netmap.NodeInfo) {
	// FIXME:
	//  status: code = 1024 message = owner mismatches signature
	partObj, err := iec.FormObjectForECPartWithoutSession(p.signer, parent, part, iec.PartInfo{
		RuleIndex: ruleIdx,
		Index:     partIdx,
	})
	if err != nil {
		...
		if p.network.IsLocalNodePublicKey(sortedNodes[i].PublicKey()) {
			...
			if err == nil {
				p.log.Info("EC part successfully recreated and put to local storage",
					zap.Stringer("container", parent.GetContainerID()), zap.Stringer("parent", parent.GetID()),
					zap.Stringer("rule", rule), zap.Int("part_idx", partIdx))
				return
			}
...
// TODO: copy-paste from GET service. Share?
