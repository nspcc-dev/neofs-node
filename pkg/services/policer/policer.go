...
type (
	...
	MetricsCollector interface {
		// SetEngineConsistency must update storage engine's consistency metric
		// based on [Policer]'s check progress. `true` is for the consistent
		// state, `false` for the opposite one.
		SetEngineConsistency(bool)
	}
)
