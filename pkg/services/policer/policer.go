...
type Policer struct {
	*cfg
	...
	checkECPartsTaskCh     chan checkECPartsTask
	checkECPartsWorkerPool *ants.Pool

	recreateECPartsTaskCh     chan recreateECPartsTask
	recreateECPartsWorkerPool *ants.Pool
}
...
func New(signer neofscrypto.Signer, opts ...Option) *Policer {
	checkECPartsWorkerPool, err := ants.NewPool(100, ants.WithNonblocking(true)) // TODO: config
	if err != nil {
		panic(fmt.Errorf("ants.NewPool: %w", err))
	}

	recreateECPartsWorkerPool, err := ants.NewPool(100, ants.WithNonblocking(true)) // TODO: config
	if err != nil {
		panic(fmt.Errorf("ants.NewPool: %w", err))
	}
	...
	return &Policer{
		...
		signer:                    signer,
		checkECPartsTaskCh:        make(chan checkECPartsTask, 100),
		checkECPartsWorkerPool:    checkECPartsWorkerPool,
		recreateECPartsTaskCh:     make(chan recreateECPartsTask, 100),
		recreateECPartsWorkerPool: recreateECPartsWorkerPool,
	}
}
...
func (x *remoteHeader) headObject(ctx context.Context, node netmapsdk.NodeInfo, addr oid.Address, checkOID bool, xs []string) (object.Object, error) {
	...
	hdr, err := x.RemoteHeader.Head(ctx, &p)
	if err != nil {
