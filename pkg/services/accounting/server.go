...
func (s *server) makeBalanceResponse(body *protoaccounting.BalanceResponse_Body, code uint32, msg string) (*protoaccounting.BalanceResponse, error) {
	v := version.Current()
...
func (s *server) makeFailedBalanceResponse(code uint32, err error) (*protoaccounting.BalanceResponse, error) {
	return s.makeBalanceResponse(nil, code, err.Error())
}
...
func (s *server) Balance(_ context.Context, req *protoaccounting.BalanceRequest) (*protoaccounting.BalanceResponse, error) {
	...
	if err := signature.VerifyServiceMessage(balReq); err != nil {
		return s.makeFailedBalanceResponse(1026, err)
	}

	mUsr := req.GetBody().GetOwnerId()
	if mUsr == nil {
		return s.makeFailedBalanceResponse(1024, errors.New("missing account"))
	}
	...
	if err := id.ReadFromV2(id2); err != nil {
		return s.makeFailedBalanceResponse(1024, fmt.Errorf("invalid account: %w", err))
	}

	bal, err := s.contract.BalanceOf(id)
	if err != nil {
		return s.makeFailedBalanceResponse(1024, err)
	}
	...
	return s.makeBalanceResponse(body, 0, "")
}
