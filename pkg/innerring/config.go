...
func validateBlockchainConfig(cfg *config.Config) error {
	...
	magicKey := cfgPathFSChainLocalConsensus + ".magic"
	if !cfg.IsSet(magicKey) {
		return fmt.Errorf("missing section '%s'", magicKey)
	}
	if cfgFSChainLocalConsensus.Magic == 0 {
		return fmt.Errorf("magic can't be zero '%s'", magicKey)
	}

	var (
		storageSection = cfgPathFSChainLocalConsensus + ".storage"
		storageTypeKey = storageSection + ".type"
	)
	if !cfg.IsSet(storageSection) {
		return fmt.Errorf("missing storage section '%s'", storageSection)
	}
	if !cfg.IsSet(storageTypeKey) {
		return fmt.Errorf("missing storage type '%s'", storageTypeKey)
	}
	...
	default:
		return fmt.Errorf("unsupported storage type '%s': '%s'", storageTypeKey, typ)
	}

	var committeeKey = cfgPathFSChainLocalConsensus + ".committee"
	if !cfg.IsSet(committeeKey) {
		return fmt.Errorf("missing committee section '%s'", committeeKey)
	}
	...
	if cfg.IsSet(hardForksKey) {
		for k := range cfgFSChainLocalConsensus.Hardforks.Name {
			if k == "" {
				return fmt.Errorf("missing hardforks name '%s'", hardForksKey)
			}
		}
	}

	if cfg.IsSet(cfgPathFSChainLocalConsensus + ".validators_history") {
		...
		for k, v := range cfgFSChainLocalConsensus.ValidatorsHistory.Height {
			...
			if v <= 0 {
				return fmt.Errorf("value %d is out of allowable range", v)
