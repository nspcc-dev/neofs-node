...
func objectIDFromStackItem(i stackitem.Item) ([]byte, error) {
	oID, ok := i.Value().([]byte)
	if !ok {
		return nil, fmt.Errorf("unexpected argument value: %T expected, %T given", oID, i.Value())
	}
	if len(oID) != smartcontract.Hash256Len {
		return nil, fmt.Errorf("unexpected object ID length: %d expected, %d given", smartcontract.Hash256Len, len(oID))
	}

	return oID, nil
}

func containerIDFromStackItem(i stackitem.Item) []byte {
	cID, ok := i.Value().([]byte)
	if !ok {
		panic(fmt.Errorf("unexpected argument value: %T expected, %T given", cID, i.Value()))
	}
