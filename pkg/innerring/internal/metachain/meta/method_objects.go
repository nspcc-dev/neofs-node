...
func putStorageItem(ic *interop.Context, key, value []byte) {
	ic.DAO.PutStorageItem(MetaDataContractID, key, value)
}

func getStorageItem(ic *interop.Context, key []byte) state.StorageItem {
	return ic.DAO.GetStorageItem(MetaDataContractID, key)
}

func deleteStorageItem(ic *interop.Context, key []byte) {
	ic.DAO.DeleteStorageItem(MetaDataContractID, key)
}

func storeObject(ic *interop.Context, o objectMeta) error {
	// temporary solution, there is not enough space to store a full object
	// address in DB according to enforced restrictions on the keys; it is
	// likely that there will be a one-to-one chain for every container, and
	// it will be ok to store objectID only
	key := make([]byte, limits.MaxStorageKeyLen)
	copy(key[1:], o.cID)

	if o.deleted != nil {
		key[0] = lockedByIndex
		copy(key[1+32:], o.deleted)
		...
		key[0] = typeIndex
		deleteStorageItem(ic, key)
		key[0] = firstPartIndex
		deleteStorageItem(ic, key)
		key[0] = previousPartIndex
		deleteStorageItem(ic, key)
		key[0] = deletedIndex
		deleteStorageItem(ic, key)
		key[0] = lockedIndex
	...
	key[0] = addrIndex
	putStorageItem(ic, key, []byte{})

	sizeRaw := make([]byte, 8)
	binary.LittleEndian.PutUint64(sizeRaw, o.size)
	key[0] = sizeIndex
	putStorageItem(ic, key, sizeRaw)

	key[0] = typeIndex
	putStorageItem(ic, key, []byte{o.typ})

	if o.firstPart != nil {
		key[0] = firstPartIndex
		putStorageItem(ic, key, o.firstPart)
	}

	if o.previousPart != nil {
		key[0] = previousPartIndex
		putStorageItem(ic, key, o.previousPart)
	}

	if o.deleted != nil {
		key[0] = deletedIndex
		putStorageItem(ic, key, o.deleted)
	}

	if o.locked != nil {
		key[0] = lockedIndex
		putStorageItem(ic, key, o.locked)

		key[0] = lockedByIndex
		copy(key[1+32:], o.locked)
		putStorageItem(ic, key, o.oID)
	}
