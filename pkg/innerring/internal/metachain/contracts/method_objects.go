...
func (m MetaData) submitObjectPut(ic *interop.Context, args []stackitem.Item) stackitem.Item {
	...
	sigsVectors, ok := args[1].Value().([]stackitem.Item)
	if !ok {
	...
	for i := range sigsVectors {
		vectorRaw, ok := sigsVectors[i].Value().([]stackitem.Item)
		if !ok {
			panic(fmt.Errorf("unexpected %d signatures vector value: %T expected, %T given", i, vectorRaw, sigsVectors[i].Value()))
		}
		vector := make([][]byte, 0, len(vectorRaw))
		for j := range vectorRaw {
			sig, ok := vectorRaw[j].Value().([]byte)
			if !ok {
				panic(fmt.Errorf("unexpected %d signature value in %d signatures vector: %T expected, %T given", j, i, sig, sigsVectors[j].Value()))
			}
	...
	if v, ok := getFromMap(metaInfo, "locked"); ok {
		...
		for i, lRaw := range locked {
			...
			if len(l) != smartcontract.Hash256Len {
				panic(fmt.Errorf("invalid locked object length: %d", len(l)))
			}
		}
	}
	if v, ok := getFromMap(metaInfo, "deleted"); ok {
		...
		for i, dRaw := range deleted {
			...
			if len(d) != smartcontract.Hash256Len {
				panic(fmt.Errorf("invalid deleted object length: %d", len(d)))
			}
		}
	}
