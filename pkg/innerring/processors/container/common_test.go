...
func (x testAuthSystem) VerifySignature(usr user.ID, data []byte, signature []byte, key *neofsecdsa.PublicKeyRFC6979) error {
	require.Equal(x.t, x.input.ownerContainer, usr)
...
func TestVerifySignature(t *testing.T) {
	...
	p.authSystem = authSystem

	input := signatureVerificationData{
		ownerContainer: *usertest.ID(),
