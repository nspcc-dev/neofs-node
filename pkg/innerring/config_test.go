...
// YAML configuration of the IR consensus with all required fields.
const validBlockchainConfigMinimalDeprecated = `
morph:
  consensus:
    magic: 15405
    committee:
...
func TestParseBlockchainConfig(t *testing.T) {
	...
	t.Run("minimal with deprecated 'morph'", func(t *testing.T) {
		cfg := _newConfigFromYAML(t, validBlockchainConfigMinimalDeprecated, "")
	...
	t.Run("incomplete", func(t *testing.T) {
		for _, requiredKey := range []string{
			...
			cfg := newValidBlockchainConfig(t, !fullConfig)
			cfg.Unset("fschain.consensus." + requiredKey)
			err := validateBlockchainConfig(cfg)
