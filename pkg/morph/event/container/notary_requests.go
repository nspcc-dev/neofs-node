...
func RestoreRemoveAttributeRequest(notaryReq event.NotaryEvent) (event.Event, error) {
	...
	if res.InvocationScript, err = getValueFromArg(args, argNum-3, "invocation script", stackitem.ByteArrayT, event.BytesFromOpcode); err != nil {
		return nil, err
	}
	if res.VerificationScript, err = getValueFromArg(args, argNum-4, "verification script", stackitem.ByteArrayT, event.BytesFromOpcode); err != nil {
		return nil, err
	}
	if res.SessionToken, err = getValueFromArg(args, argNum-5, "session token", stackitem.ByteArrayT, event.BytesFromOpcode); err != nil {
		return nil, err
	}
