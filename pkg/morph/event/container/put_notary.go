...
func parsePutNotary(ev *Put, raw *payload.P2PNotaryRequest, args []scparser.Instruction, t event.NotaryType) error {
	switch l := len(args); l {
	case putArgCnt + 3:
...
func parseNamedArgs(ev *Put, args []scparser.Instruction) error {
	var err error
	ev.name, err = scparser.GetStringFromInstr(args[0])
	if err != nil {
		return fmt.Errorf("zone: %w", err)
	}

	ev.zone, err = scparser.GetStringFromInstr(args[1])
	if err != nil {
