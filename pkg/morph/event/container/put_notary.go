...
func parsePutNotary(ev *Put, raw *payload.P2PNotaryRequest, args []scparser.Instruction) error {
	switch l := len(args); l {
	case putArgCnt + 3:
		...
		enableMeta, err := scparser.GetBoolFromInstr(args[l-1])
		if err != nil {
	...
	for i, arg := range args[:putArgCnt] {
		b, err := scparser.GetBytesFromInstr(arg)
		if err != nil {
...
func parseNamedArgs(ev *Put, args []scparser.Instruction) error {
	name, err := scparser.GetStringFromInstr(args[0])
	if err != nil {
		return fmt.Errorf("parse arg zone: %w", err)
	}

	zone, err := scparser.GetStringFromInstr(args[1])
	if err != nil {
