...
type (
	...
	corePayloadRangeReceiver struct {
		...
		relRecv     _range.RelativeReceiver
...
const emHeadRecvFail = "could not receive %d of %d object head"

var (
	childrenNotFound = errors.New("could not find child objects")
	errNonAssembly   = errors.New("node is not capable to assemble the object")
)
...
func (s *coreObjectReceiver) getObject(ctx context.Context, info ...transport.GetInfo) (*objectData, error) {
	...
	ctx = contextWithValues(ctx,
		...
		storagegroup.BearerToken, info[0].GetBearerToken(),
		storagegroup.ExtendedHeaders, info[0].ExtendedHeaders(),
	)
...
func (s *corePayloadRangeReceiver) getRangeData(ctx context.Context, info transport.RangeInfo, selection ...Object) (res io.Reader, err error) {
	...
	chopper, err = s.chopTable.GetChopper(addr, _range.RCCharybdis)
	if err != nil || !chopper.Closed() {
		if len(selection) == 0 {
			if chopper, err = s.chopTable.GetChopper(addr, _range.RCScylla); err != nil {
				if chopper, err = _range.NewScylla(&_range.ChopperParams{
			...
			if chopper, err = _range.NewCharybdis(&_range.CharybdisParams{
				Addr:           addr,
				ReadySelection: rs,
			}); err != nil {
				return
...
func bearerFromContext(ctx context.Context) service.BearerToken {
	if v, ok := ctx.Value(storagegroup.BearerToken).(service.BearerToken); ok {
		return v
	}
...
func extendedHeadersFromContext(ctx context.Context) []service.ExtendedHeader {
	if v, ok := ctx.Value(storagegroup.ExtendedHeaders).([]service.ExtendedHeader); ok {
		return v
	}
