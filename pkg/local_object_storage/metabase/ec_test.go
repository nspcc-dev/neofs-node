...
func BenchmarkDB_ResolveECPartWithPayloadLen(b *testing.B) {
	...
	for b.Loop() {
		_, err = db.ResolveECPartWithPayloadLen(cnr, parentID, pi)
		require.NoError(b, err)
	}
}
...
func TestDB_ResolveECPart(t *testing.T) {
	...
	checkOK := func(t *testing.T, db *meta.DB, pi iec.PartInfo, exp oid.ID) {
		...
		idLen, err := db.ResolveECPartWithPayloadLen(cnr, parentID, pi)
		require.NoError(t, err)
		require.Equal(t, exp, idLen.ID)
		require.EqualValues(t, 1000+pi.Index, idLen.PayloadLen)
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.typ.String(), func(t *testing.T) {
			...
			idLen, err := db.ResolveECPartWithPayloadLen(cnr, sysObj.GetID(), pi)
			require.NoError(t, err)
			require.Equal(t, sysObj.GetID(), idLen.ID)
			require.EqualValues(t, sysObj.PayloadSize(), idLen.PayloadLen)
		})
	}
