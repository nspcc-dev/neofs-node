...
func selectNFromBucket(bkt *bbolt.Bucket, // main bucket
	...
	var (
		...
		count      = len(to)
		offset     []byte
		phyPrefix  = make([]byte, 1+len(object.FilterPhysical)+1+len(binPropMarker)+1)
		typePrefix = make([]byte, metaIDTypePrefixSize)
	)
	phyPrefix[0] = metaPrefixAttrIDPlain
	copy(phyPrefix[1:], object.FilterPhysical)
	copy(phyPrefix[1+len(object.FilterPhysical)+1:], binPropMarker)
