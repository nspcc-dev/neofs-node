...
func deleteMetadata(c *bbolt.Cursor, l *zap.Logger, cnr cid.ID, id oid.ID, isParent bool) (objectDiff, error) {
	...
	if gcK != nil {
		diff.GC--
		err = updateCounter(metaBkt, gcCounter, 1, false)
		if err != nil {
	...
	if !nonPhy {
		diff.Phy--
		err = updateCounter(metaBkt, phyCounter, 1, false)
		if err != nil {
	...
	case object.TypeRegular:
		if isRoot {
			diff.Root--
			err = updateCounter(metaBkt, rootCounter, 1, false)
		}
	case object.TypeTombstone:
		diff.TS--
		err = updateCounter(metaBkt, tsCounter, 1, false)
	case object.TypeLink:
		diff.Link--
		err = updateCounter(metaBkt, linkCounter, 1, false)
	case object.TypeLock:
		diff.Lock--
		err = updateCounter(metaBkt, lockCounter, 1, false)
	default:
	}
	if err != nil {
...
func collectChildren(cnrMetaCrs *bbolt.Cursor, cnr cid.ID, parentID oid.ID) ([]oid.ID, error) {
	...
	if errors.As(parInfo, &siErr) {
		...
		case !firstID.IsZero():
			...
			res = slices.AppendSeq(res, iterAttrVal(cnrMetaCrs, object.FilterFirstSplitObject, firstID[:]))
		case splitID != nil:
			res = slices.AppendSeq(res, iterAttrVal(cnrMetaCrs, object.FilterSplitID, splitID.ToV2()))
		default:
