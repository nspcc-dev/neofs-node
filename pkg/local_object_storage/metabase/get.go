...
func get(tx *bbolt.Tx, addr oid.Address, checkStatus, raw bool, currEpoch uint64) (*objectSDK.Object, error) {
	...
	for ak, _ := objCur.Seek(objPrefix); bytes.HasPrefix(ak, objPrefix); ak, _ = objCur.Next() {
		...
		case objectSDK.FilterVersion:
			...
			if len(attrVal) < 4 {
				return nil, fmt.Errorf("invalid version in meta of %s/%s: wrong length", cnr, objID)
			}
			if attrVal[0] != 'v' {
				return nil, fmt.Errorf("invalid version in meta of %s/%s: doesn't start with 'v'", cnr, objID)
			}
			majB, minB, found := bytes.Cut(attrVal[1:], []byte("."))
			if !found {
				return nil, fmt.Errorf("invalid version in meta of %s/%s: dot not found", cnr, objID)
			}
			majV, err := strconv.ParseUint(string(majB), 10, 32)
			if err != nil {
				return nil, fmt.Errorf("invalid major version in meta of %s/%s: %w", cnr, objID, err)
			}
			minV, err := strconv.ParseUint(string(minB), 10, 32)
			if err != nil {
				return nil, fmt.Errorf("invalid minor version in meta of %s/%s: %w", cnr, objID, err)
			}
			v.SetMajor(uint32(majV))
			v.SetMinor(uint32(minV))
			obj.SetVersion(&v)
