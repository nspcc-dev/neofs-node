...
// TODO: docs
func (s *Shard) GetBytes(addr oid.Address, alloc func(ln int) []byte) ([]byte, error) {
	b, _, err := s.getBytesWithMetadataLookup(addr, true /* skipMeta */, alloc)
	return b, err
}

// TODO: docs
func (s *Shard) GetBytesWithMetadataLookup(addr oid.Address, alloc func(ln int) []byte) ([]byte, bool, error) {
	return s.getBytesWithMetadataLookup(addr, false /* skipMeta */, alloc)
}

func (s *Shard) getBytesWithMetadataLookup(addr oid.Address, skipMeta bool, alloc func(ln int) []byte) ([]byte, bool, error) {
	s.m.RLock()
	defer s.m.RUnlock()
	...
	hasMeta, err := s.fetchObjectData(addr, skipMeta, func(bs *blobstor.BlobStor, subStorageID []byte) error {
		var err error
		b, err = bs.GetBytes(addr, nil /* subStorageID */, allocReuse)
		return err
	}, func(w writecache.Cache) error {
		var err error
		b, err = w.GetBytes(addr, allocReuse)
		return err
	})
