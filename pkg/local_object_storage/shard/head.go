...
func (s *Shard) ReadHeader(addr oid.Address, raw bool, buf []byte) (int, error) {
	...
	for _, child := range children {
		...
		if s.hasWriteCache() {
			...
			if !errors.Is(err, apistatus.ErrObjectNotFound) {
				s.log.Warn("ReadHeader for child from writecache failed",
					zap.Stringer("addr", addr), zap.Stringer("child", child), zap.Error(err))
			}
		}
		...
		s.log.Warn("ReadHeader for child from blobstor failed",
			zap.Stringer("addr", addr), zap.Stringer("child", child), zap.Error(err))
	}
	...
	if s.hasWriteCache() {
		...
		if !errors.Is(err, apistatus.ErrObjectNotFound) {
			s.log.Warn("ReadHeader from writecache failed",
