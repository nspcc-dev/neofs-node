...
type resolveECPartWithLenValue struct {
	idLen iobject.OIDWithPayloadLen
	err   error
}
...
func (x *mockMetabase) ResolveECPartWithPayloadLen(cnr cid.ID, parent oid.ID, pi iec.PartInfo) (iobject.OIDWithPayloadLen, error) {
	...
	if !ok {
		return iobject.OIDWithPayloadLen{}, errors.New("[test] unexpected part requested")
	}
	return val.idLen, val.err
}
