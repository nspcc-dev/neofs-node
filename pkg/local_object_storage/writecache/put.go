...
//
// If c is in read-only mode now, InitPut returns [ErrReadOnly].
func (c *cache) InitPut(addr oid.Address, headerLen uint64, payloadLen uint64, headerW io.WriterTo) (io.WriteCloser, func(), error) {
	...
	if err != nil {
		c.metrics.addInitPutDuration(time.Since(st))
		return nil, nil, err
	}
...
func (x *objectPayloadWriteStream) submitMetrics() {
	x.writeCache.metrics.addInitPutDuration(time.Since(x.startTime))
}
