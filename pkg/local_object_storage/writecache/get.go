...
func Get(db *bbolt.DB, key []byte) ([]byte, error) {
	return get(db, key, nil)
}

func get(db *bbolt.DB, key []byte, alloc func(ln int) []byte) ([]byte, error) {
	var value []byte
...
func (c *cache) GetBytes(addr oid.Address, alloc func(ln int) []byte) ([]byte, error) {
	saddr := addr.EncodeToString()

	b, err := get(c.db, []byte(saddr), alloc)
	if err == nil {
		c.flushed.Get(saddr)
		return b, nil
	}

	b, err = c.fsTree.GetBytes(addr, func(ln int) []byte {
		if cap(b) >= ln {
			return b[:ln]
		}
		if alloc != nil {
	...
	if err != nil {
		return b, logicerr.Wrap(apistatus.ObjectNotFound{})
	}
