...
func (e *StorageEngine) GetBytes(addr oid.Address, alloc func(ln int) []byte) ([]byte, error) {
	var b []byte
	var err error
	...
	err = e.execIfNotBlocked(func() error {
		return e.get(addr, func(s *shard.Shard, ignoreMetadata bool) (hasMetadata bool, err error) {
			if ignoreMetadata {
				b, err = s.GetBytes(addr, allocReuse)
			} else {
				b, hasMetadata, err = s.GetBytesWithMetadataLookup(addr, alloc)
			}
			return
		})
	})
