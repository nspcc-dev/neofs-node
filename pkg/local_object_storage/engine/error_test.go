...
func newEngine(t testing.TB, dir string, opts ...Option) (*StorageEngine, string, [2]*coreshard.ID) {
	if dir == "" {
		dir = t.TempDir()
	}
...
func newEngineWithErrorThreshold(t testing.TB, dir string, errThreshold uint32) (*StorageEngine, string, [2]*coreshard.ID) {
...
func checkShardState(t *testing.T, e *StorageEngine, id *coreshard.ID, errCount uint32, mode mode.Mode) {
	e.mtx.RLock()
