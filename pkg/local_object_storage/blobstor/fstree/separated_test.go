...
func TestSeparatedObject(t *testing.T) {
	...
	t.Run("streaming put", func(t *testing.T) {
		tree := setupFSTree(t)
		obj := objecttest.Object()
		...
		stream, _, err := tree.InitPut(obj.Address(), uint64(len(header)), uint64(len(payload)), bytes.NewReader(header))
		require.NoError(t, err)
		...
		raw := readRawObjectFile(t, tree, obj.Address())
		headerLen, payloadLen := parseSeparatedPrefix(raw)
		require.EqualValues(t, len(obj.Marshal())-len(payload), headerLen)
		require.EqualValues(t, len(payload), payloadLen)
