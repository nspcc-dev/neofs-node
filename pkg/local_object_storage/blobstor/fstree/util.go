...
func newPayloadReadSeekCloser(reader io.ReadSeekCloser) *payloadReadSeekCloser {
...
func (p *payloadReadSeekCloser) Seek(offset int64, whence int) (int64, error) {
	...
	if _, err := p.reader.Seek(skip, io.SeekCurrent); err != nil {
		return p.pos, fmt.Errorf("seek payload stream: %w", err)
	}
	p.pos += skip
