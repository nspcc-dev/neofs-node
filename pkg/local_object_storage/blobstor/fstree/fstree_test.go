...
func separateHeaderAndPayload(header []byte, payload []byte) []byte {
	payloadTag := []byte(nil)
	if len(payload) > 0 {
		payloadLenBuf := make([]byte, binary.MaxVarintLen64)
		n := binary.PutUvarint(payloadLenBuf, uint64(len(payload)))
		payloadTag = append([]byte{iprotobuf.TagBytes4}, payloadLenBuf[:n]...)
	}

	return slices.Concat(separatedObjectPrefix(uint64(len(header)+len(payloadTag)), uint64(len(payload))), header, payloadTag, payload)
}
