...
func (t *FSTree) reshape(ctx context.Context) (int, error) {
	...
	if err := t.checkSecondaryFiles(ctx, t.RootPath, 0, ""); err != nil {
		return moved, err
	}
	return moved, nil
}

func (t *FSTree) checkSecondaryFiles(ctx context.Context, dir string, depth uint64, prefix string) error {
	...
	for _, entry := range entries {
		...
		if depth < t.secondaryDepth {
			if entry.IsDir() {
				if err := t.checkSecondaryFiles(ctx, path, depth+1, prefix+entry.Name()); err != nil {
					return err
				}
			}
			continue
		...
		if !entry.IsDir() {
			if _, err := addressFromString(prefix + entry.Name()); err == nil {
				return fmt.Errorf("secondary layout still contains object file %q", path)
			}
		}
	}
	return nil
