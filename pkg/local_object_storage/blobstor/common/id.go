...
// Implements the built-in comparable interface.
type ID struct {
	raw  [IDSize]byte
	str  string
	hash uint64
...
func DecodeIDString(s string) (ID, error) {
	if s == "" {
		return ID{}, errors.New("empty shard ID string")
	}
	...
	if len(b) != IDSize {
		return ID{}, fmt.Errorf("invalid shard ID length %d, expected %d", len(b), IDSize)
	}
