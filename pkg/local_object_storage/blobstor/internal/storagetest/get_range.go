...
func TestGetRangeStream(t *testing.T, cons Constructor, minSize, maxSize uint64) {
	...
	t.Run("missing object", func(t *testing.T) {
		_, _, _, err := s.GetRangeStream(oidtest.Address(), blobstor.NewPayloadRange(0, 1), false)
	...
	testGetRangeStream := func(t *testing.T, objects []objectDesc) {
		...
		t.Run("regular", func(t *testing.T) {
			_, pldLen, stream, err := s.GetRangeStream(objects[0].addr, blobstor.NewPayloadRange(start, stop-start), false)
		...
		t.Run("offset > len(payload)", func(t *testing.T) {
			_, _, _, err := s.GetRangeStream(objects[0].addr, blobstor.NewPayloadRange(uint64(len(payload)+10), 10), false)
			require.ErrorAs(t, err, new(apistatus.ObjectOutOfRange))
		})

		t.Run("offset + length > len(payload)", func(t *testing.T) {
			_, _, _, err := s.GetRangeStream(objects[0].addr, blobstor.NewPayloadRange(10, uint64(len(payload))), false)
			require.ErrorAs(t, err, new(apistatus.ObjectOutOfRange))
		})

		t.Run("length is negative when converted to int64", func(t *testing.T) {
			_, _, _, err := s.GetRangeStream(objects[0].addr, blobstor.NewPayloadRange(0, 1<<63), false)
			require.ErrorAs(t, err, new(apistatus.ObjectOutOfRange))
		})

		t.Run("offset + length overflow uint64", func(t *testing.T) {
			_, _, _, err := s.GetRangeStream(objects[0].addr, blobstor.NewPayloadRange(10, math.MaxUint64-2), false)
			require.ErrorAs(t, err, new(apistatus.ObjectOutOfRange))
		})

		t.Run("zero range", func(t *testing.T) {
			for i := range objects {
				_, pldLen, stream, err := s.GetRangeStream(objects[i].addr, blobstor.NewPayloadRange(0, 0), false)
				require.NoError(t, err)
