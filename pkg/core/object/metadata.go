		...
		if idIter {
			...
			for i := range fs {
				...
				if IsIntegerSearchOp(mch) {
					f := fInt[i]
					matches = f.AutoMatch || intBytesMatch(primDBVal, mch, f.RawValue)
				} else {
		...
		for i := range fs {
			...
			for j := i; j < len(fs); j++ {
				...
				if IsIntegerSearchOp(m) {
					...
					if dbValIsInt {
						f := fInt[j]
						matches = f.AutoMatch || intMatches(dbValInt, m, f.IntValue)
					}
				} else {
		...
		if cursor != "" {
			...
			if IsIntegerSearchOp(primMatcher) {
				...
				if !f.AutoMatch && (primMatcher == object.MatchNumGE || primMatcher == object.MatchNumGT) {
					primSeekKey = slices.Concat([]byte{metaPrefixAttrIDInt}, []byte(attrs[0]), AttributeDelimiter, f.RawValue)
					primKeysPrefix = primSeekKey[:1+len(attrs[0])+attributeDelimiterLen]
				} else {
