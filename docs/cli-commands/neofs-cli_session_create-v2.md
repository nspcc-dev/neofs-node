...
```
  ...
  -e, --expire-at uint        Expiration time in Unix timestamps for token to stay valid
      --final                 Set the final flag in the token, disallowing further delegation
  -h, --help                  help for create-v2
      --json                  Output token in JSON
  -l, --lifetime uint         Duration in Unix timestamps for token to stay valid (default 36000)
