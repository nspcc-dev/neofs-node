...
- `body`: Contains:
  ...
  - `nonce`: A random 32-bit unsigned integer to protect against replay attacks. Each request contains a unique nonce.
...
The response is a JSON object containing the verification result as a raw JSON message in the `result`
...
```json
{
  ...
  "signature": {
    "sign": "<bytes: signed result>"
  }
}
```

- `result`: A JSON-encoded object with the following fields:
...
- `signature.sign`: Signature bytes over the raw `result` field.
...
- The signature must be valid for the raw JSON-encoded `result` field.
...
verification result in the `result` field, including the same nonce.
5. IR node verifies the response signature (over the raw `result` field), validates that the nonce matches, 
