...
| `cache_ttl`           | `duration` | `15s`         | Sidechain cache TTL value (min interval between similar calls).<br/>Negative value disables caching.<br/>Cached entities: containers, container lists, eACL tables. |
| `quota_ttl`           | `duration  | `10m`         | Container used space quotas cache TTL. Negative value disables caching.                                                                                             |
...
| `search_iteration_limit` | `int`      | `10000`       | Maximum number of consecutive objects to iterate without a match during search operations. `0` means no limit. |
